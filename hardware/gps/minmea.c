    ...
    while (*format) {
        ...
        switch (type) {
            ...
            case 't': { // NMEA talker+sentence identifier (char *).
                ...
                if(field[1] == 'P' && field[2] == 'T' && field[3] == 'W' && field[4] == 'S')
                {
                    char *buf = va_arg(ap, char *);
                    memcpy(buf, field+3, 5);
                    buf[5] = '\0';
                }
                else
                {
...
enum minmea_sentence_id minmea_sentence_id(const char *sentence, bool strict)
{
    ...
    if (!strcmp(type+2, "EPE"))
        return MINMEA_SENTENCE_EPE;
    if (!strcmp(type+2, "VER"))
        return MINMEA_SENTENCE_VER;

    return MINMEA_UNKNOWN;
}

//$PTWSVER,TELIT,V13-2.3.0-STD-5.1.5-N96-000200*78
bool minmea_parse_ver(struct minmea_sentence_ver *frame, const char *sentence)
{
    // $GPRMC,081836,A,3751.65,S,14507.36,E,000.0,360.0,130998,011.3,E*62
    char type[6];

    if (!minmea_scan(sentence, "tss",
            type,
            &frame->brand,
            &frame->vers))
        return false;
...
bool minmea_parse_epe(struct minmea_sentence_epe *frame, const char *sentence)
{
    // $PTWSEPE,132517.000,5.35,16.74*56
    char type[6];
    int latitude_direction;
    int longitude_direction;

    if (!minmea_scan(sentence, "tTff",
            type,
            &frame->time,
            &frame->ehpe,
            &frame->evpe))
...
bool minmea_parse_gsv(struct minmea_sentence_gsv *frame, const char *sentence)
{
    ...
    memset(frame,0,sizeof(struct minmea_sentence_gsv));
